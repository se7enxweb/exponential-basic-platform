<?php
//
// $id: imageedit.php 9571 2002-05-29 13:55:23Z br $
//
...
if ( $action == "Insert" || $action == "Update" )
{
    ...
    if ( $fileCheck )
    {
        ...
        if ( $file->getUploadedFile( "userfile" ) )
        {
            ...
            if ( $imageTest->checkImage( $file ) && $imageTest->setImage( $file ) )
            {
                $fileOK = true;
            }
            else
...
if ( $action == "Insert" && $error == false )
{
    ...
    if ( !is_array( $categoryArray ) ) $categoryArray = [];
    $categories = array_unique( array_merge( $categoryArray, array( $categoryID ) ) );
    foreach ( $categories as $categoryItem )
    {
        if ( eZObjectPermission::hasPermission( $categoryItem, "imagecatalogue_category", 'u' ) )
        {
            eZImageCategory::addImage( $image, $categoryItem );
        }
    }

    eZPBLog::writeNotice( "Picture added to catalogue: {$image->name()} from IP: {$_SERVER['REMOTE_ADDR']}" );
