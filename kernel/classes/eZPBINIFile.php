XSS<?php
//
// $Id: eZPBINIFile.php 9625 2002-06-11 08:36:33Z jhe $
//
...
//!! eZCommon
//! The eZPBINIFile class provides .ini file functions.
/*!
  ...
  \code
  include_once( "kernel/classes/eZPBINIFile.php" );
...
class eZPBINIFile
{
    ...
    */
    function __construct( $inifilename = "", $write = false )
    {
        ...
        $origTime = eZPBFile::filemtime( $inifilename );
        $overrideTime = eZPBFile::filemtime( "settings/override/" . basename($inifilename) );
        ...
        if ( file_exists( $cachedFile ) )
        {
            ...
            if ( $cacheTime < $origTime )
                $loadCache = false;
            if ( file_exists( "settings/override/" . basename($inifilename) ) and $cacheTime < $overrideTime )
                $loadCache = false;
            if ( file_exists( "settings/override/" . basename($inifilename) . ".append" ) and $cacheTime < $appendTime )
                $loadCache = false;
        }
    ...
    function load_data( $inifilename = "", $write = true, $useoverride = true )
    {
        ...
        if ( !empty( $inifilename ) )
        {
            ...
            {
                if ( file_exists( $inifilename . ".php") )
                {
            ...
            else
	    	{
				if ( $inifilename == "settings/site.ini" ) 
				{
		   			...
		   			if ( file_exists( "settings/override/$filesiteini" ) ) 
		   			{
		       			if ( file_exists( "settings/override/$filesiteini" . ".php") ) 
		       			{
			   		...
			   		{
			     		if ( file_exists( "settings/$filesiteini" ) ) 
			     		{
			       			if ( file_exists( "settings/$filesiteini" . ".php") )
			       			{
    ...
    */
    function parse( $inifilename, $append = false )
    {
        ...
        $fp = eZPBFile::fopen( $inifilename, $this->WRITE_ACCESS ? "r+" : "r" );
        ...
        $contents = fread( $fp, eZPBFile::filesize( $inifilename ) );
    ...
    */
    function save_data()
    {
        $fp = eZPBFile::fopen( $this->INI_FILE_NAME, "w" );
